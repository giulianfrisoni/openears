...
An ad-hoc signed Apple-silicon preview DMG can be produced with `sh scripts/package-dmg.sh`. Developer ID signing, Apple notarization and website hosting remain separate work.
...
3. The landing page links to the real repository. Link binary downloads to the matching GitHub release.
...
5. For binary releases, distribute the corresponding source, LICENSE, ACKNOWLEDGMENTS.md and vendored notices together. The preview DMG includes the license, acknowledgments and compatibility notes, while the app bundle contains vendored notices. The local app is ad-hoc signed; Developer ID signing and notarization are still needed for ordinary public distribution.
