...
public final class Device: NSObject {
    ...
    private let acceptsName: (String) -> Bool
    ...
    public init(_ callback: Callback, acceptsName: @escaping (String) -> Bool = { _ in true }) {
        self.callback = callback
        self.acceptsName = acceptsName
        super.init()
...
extension Device {
    ...
    public func checkAndConnectToExistingDevices() -> Bool {
        ...
        guard let peripheral = connectedPeripherals.first(where: { acceptsName($0.name ?? "") }) else {
            return false
        }
    ...
    public func connect(to peripheral: CBPeripheral) {
        guard acceptsName(peripheral.name ?? "") else { return }
    ...
    /// OpenEars: read state after commands instead of assuming a write succeeded.
    public func refreshControls() {
        guard isConnected else { return }
        runTasks([sendReadBatteryRequest, sendReadANCRequest, sendReadEQRequest, sendReadSpatialAudioRequest], delay: 0.15)
    }
